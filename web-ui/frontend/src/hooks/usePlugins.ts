...
// API base URL - now served from same origin (unified app)
const API_BASE_URL = '';
...
async function fetchPluginDetails(pluginId: string): Promise<InstalledPlugin> {
  const response = await fetch(`${API_BASE_URL}/api/v1/plugins/${pluginId}`);
  if (!response.ok) {
...
async function startPlugin(pluginId: string): Promise<void> {
  const response = await fetch(`${API_BASE_URL}/api/v1/plugins/${pluginId}/start`, {
    method: 'POST',
  });
  if (!response.ok) {
    const error = await response.json();
...
async function stopPlugin(pluginId: string): Promise<void> {
  const response = await fetch(`${API_BASE_URL}/api/v1/plugins/${pluginId}/stop`, {
    method: 'POST',
  });
  if (!response.ok) {
    const error = await response.json();
...
async function restartPlugin(pluginId: string): Promise<void> {
  const response = await fetch(`${API_BASE_URL}/api/v1/plugins/${pluginId}/restart`, {
    method: 'POST',
  });
  if (!response.ok) {
    const error = await response.json();
...
async function fetchPluginLogs(pluginId: string, tail: number = 100): Promise<string[]> {
  const response = await fetch(`${API_BASE_URL}/api/v1/plugins/${pluginId}/logs?tail=${tail}`);
  if (!response.ok) {
...
export function useAvailablePlugins() {
  return useQuery({
    ...
    queryFn: async (): Promise<RegistryPlugin[]> => {
      const response = await fetch(`${API_BASE_URL}/api/v1/registry/plugins`);
      if (!response.ok) {
